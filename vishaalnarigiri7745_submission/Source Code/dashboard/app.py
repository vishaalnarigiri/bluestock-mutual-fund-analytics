...
# Use sub_category as categories since it represents typical mutual fund selections (Large Cap, Mid Cap, Gilt, etc.)
categories = sorted(df_funds_all["sub_category"].dropna().unique())
...
def filter_df(df):
    ...
    if selected_cat:
        filtered_df = filtered_df[filtered_df["sub_category"].isin(selected_cat)]
    if selected_risk:
...
if page == "Industry Overview":
    ...
    # Apply global AMC filter
    df_aum_filtered = df_aum.copy()
    if selected_amc:
        df_aum_filtered = df_aum_filtered[df_aum_filtered["fund_house"].isin(selected_amc)]
        
    if df_aum_filtered.empty:
        st.warning("No AUM data matches the selected AMC filter. Adjust the sidebar selection.")
    else:
        ...
        with col1:
            ...
            df_aum_agg = df_aum_filtered.groupby("date")["aum_crore"].sum().reset_index()
            # Scale to Lakh Crore
            df_aum_agg["aum_lakh_crore"] = df_aum_agg["aum_crore"] / 100000.0
            fig_aum = px.line(df_aum_agg, x="date", y="aum_lakh_crore", markers=True,
                              labels={"aum_lakh_crore": "Selected AUM (Rs. Lakh Crore)", "date": "Date"},
        ...
        with col2:
            ...
            df_amc_aum = df_aum_filtered[df_aum_filtered["date"] == df_aum_filtered["date"].max()].sort_values(by="aum_crore", ascending=False)
...
# ----------------- PAGE 2: Fund Performance -----------------
    ...
    if df_perf_filtered.empty:
        st.warning("No mutual funds match the selected filters. Please adjust your Global Filters in the sidebar.")
    else:
        # Performance cards
        top_fund_row = df_perf_filtered.sort_values(by="composite_score", ascending=False).iloc[0]
        ...
        st.dataframe(
            df_perf_filtered[[
                "scheme_name", "fund_house", "sub_category", "risk_category", "expense_ratio_pct",
...
# ----------------- PAGE 3: Investor Analytics -----------------
    ...
    # Filter transaction dataset based on sidebar selection
    df_perf = load_performance_data()
    df_perf_filtered = filter_df(df_perf)
...
# ----------------- PAGE 4: SIP & Market Trends -----------------
    ...
    # Filter category inflows heatmap based on global category filters
    df_cat_filtered = df_cat.copy()
    if selected_cat:
        df_cat_filtered = df_cat_filtered[df_cat_filtered["category"].isin(selected_cat)]
    ...
    with col2:
        ...
        if df_cat_filtered.empty:
            st.warning("No category inflow matches the selected category filter. Adjust selection.")
        else:
            df_cat_pivot = df_cat_filtered.pivot(index="category", columns="month", values="net_inflow_crore")
